...
@Component({
  ...
  animations: [
    trigger("inOutAnimation", [
      transition(":enter", [
        style({ top: "-30%" }),
        animate("0.2s ease-out", style({ top: 0 }))
      ]),
      transition(":leave", [
        style({ top: 0 }),
        animate("0.2s ease-in", style({ top: "-30%" }))
      ])
    ])
  ]
})
    ...
    this.proyectoService
      ...
      .subscribe(result => {
        ...
        this.proyectos.forEach(proyecto => {
          ...
          this.proyectoService
            .getGaleriaProyecto(proyecto.id)
            .pipe(takeUntil(this.ngUnsubscribe))
            .subscribe(result => {
              ...
              proyecto.galeria.forEach(fotografia => {
                ...
                let foto: any = {};
                foto.source = this.transform(
                  "data:image/png;base64," + fotografia.archivo
                );
          ...
          this.proyectoService
            .getCategoriasProyecto(proyecto.id)
            .pipe(takeUntil(this.ngUnsubscribe))
            .subscribe(result => {
                  ...
                  .subscribe(result => {
                    ...
                    if (result.usuario.role == "Alumno") {
                      ...
                      alumno.nombreCompleto =
                        result.usuario.primer_nombre +
                        " " +
                        result.usuario.segundo_nombre +
                        " " +
                        result.usuario.primer_nombre +
                        " " +
                        result.usuario.otros_nombre +
                        " " +
                        result.usuario.primer_apellido +
                        " " +
                        result.usuario.segundo_apellido;
                      alumno.profesion = result.usuario.profesion;
                      alumno.descripcion = result.usuario.decripcion;
                      alumno.fotografia =
                        "data:image/png;base64," + result.usuario.foto;
